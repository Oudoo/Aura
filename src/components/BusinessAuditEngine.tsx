...
export function BusinessAuditEngine() {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto relative z-10">
      ...
      {step <= LEAD_STEP && (
        <div className="mb-8">
          <div className="flex justify-between text-xs text-slate font-semibold uppercase tracking-wider mb-2">
      ...
      <AnimatePresence mode="wait">
        ...
        {step < LEAD_STEP && (() => {
          ...
          return (
            <motion.div
              ...
              className="bg-void border border-fg/10 rounded-xl p-8 md:p-12"
            >
              <div className="flex items-center gap-3 mb-6">
                <div className="w-12 h-12 bg-cyan/10 rounded-full flex items-center justify-center shrink-0">
                  <Icon className="w-6 h-6 text-cyan" />
                </div>
                <span className="eyebrow text-slate uppercase tracking-wider">{item.dimensionLabel}</span>
              </div>
              <h2 className="text-2xl md:text-3xl display text-platinum mb-8 leading-tight">
                {item.question.title}
              </h2>
              <div className="space-y-3">
                {item.question.options.map((opt, i) => (
                  <button
                    ...
                    className="w-full p-4 rounded-md border border-fg/10 bg-obsidian hover:bg-cyan hover:border-cyan text-left transition-colors group flex items-center justify-between"
                  >
                    <span className="font-medium text-platinum group-hover:text-white transition-colors">{opt.text}</span>
                    <ChevronRight className="w-5 h-5 text-slate group-hover:text-white transition-colors" />
                  </button>
                ))}
              </div>
            </motion.div>
        ...
        {step === LEAD_STEP && (
          <motion.div
            ...
            transition={{ duration: 0.3, ease: "easeOut" }}
            className="bg-void border border-fg/10 rounded-xl p-8 md:p-12 text-center"
          >
            <ShieldCheck className="w-12 h-12 text-cyan mx-auto mb-6" />
            <h2 className="text-3xl display text-platinum mb-3">Your audit is ready</h2>
            ...
            <form onSubmit={handleLeadSubmit} className="space-y-4 max-w-sm mx-auto text-left">
              <input name="name" required placeholder="Full Name" className="w-full bg-obsidian border border-fg/10 rounded-md px-4 py-3 text-platinum placeholder-slate focus:outline-none focus:border-cyan transition-colors" />
              <input name="company" required placeholder="Company Name" className="w-full bg-obsidian border border-fg/10 rounded-md px-4 py-3 text-platinum placeholder-slate focus:outline-none focus:border-cyan transition-colors" />
              <input name="email" type="email" required placeholder="Work Email" className="w-full bg-obsidian border border-fg/10 rounded-md px-4 py-3 text-platinum placeholder-slate focus:outline-none focus:border-cyan transition-colors" />
              {error && (
                <p className="text-sm text-red-500 flex items-center gap-2"><AlertTriangle className="w-4 h-4" />{error}</p>
              )}
              <button type="submit" disabled={loading} className="w-full py-4 bg-cyan text-white font-semibold rounded-md hover:bg-amethyst transition-colors disabled:opacity-50 flex justify-center items-center gap-2">
        ...
        {step === RESULT_STEP && (
          <motion.div
            ...
            transition={{ duration: 0.3, ease: "easeOut" }}
            className="bg-void border border-fg/10 rounded-xl p-8 md:p-10"
          >
            <div className="text-center mb-8">
              <div className="inline-block px-4 py-1.5 rounded-full bg-cyan/10 border border-cyan/20 text-amethyst text-sm font-semibold tracking-wider uppercase mb-4">
                {tier.label} Maturity
              </div>
              <h2 className="text-5xl md:text-7xl display text-platinum mb-3">
            ...
            <div className="h-72 w-full mb-8">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={radarData} outerRadius="75%">
                  ...
                  <PolarAngleAxis dataKey="dimension" tick={{ fill: "#6A6E78", fontSize: 11 }} />
                  <PolarRadiusAxis domain={[0, 100]} tick={false} axisLine={false} />
                  <Radar dataKey="score" stroke="#0A84FF" fill="#0A84FF" fillOpacity={0.2} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
              ...
              {dimensionScores.map((d) => (
                <div key={d.id}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-slate font-semibold">{d.percent}%</span>
                  </div>
            ...
            <div className="bg-obsidian border border-fg/10 rounded-xl p-6 mb-8">
              <h3 className="text-lg font-semibold text-platinum mb-4 flex items-center gap-2">
                ...
                {gaps.map((g) => (
                  <div key={g.id}>
                    <p className="text-sm text-slate mb-2">
                      <strong className="text-platinum">{g.label}</strong> scored {g.percent}% — recommended Aura modules:
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {g.recommends.map((p) => (
                        <Link key={p.slug} href={`/products/${p.slug}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-cyan/10 text-cyan border border-cyan/20 text-sm font-medium hover:bg-cyan hover:text-white transition-colors">
            ...
            <p className="text-center text-slate text-sm mb-6">
              A senior Aura architect has been notified and will email your full gap analysis and implementation roadmap within 24 hours.
            </p>

            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link href="/products" className="inline-flex justify-center rounded-md bg-cyan text-white font-semibold hover:bg-amethyst transition-colors px-6 py-3">
                Explore the Ecosystem
              </Link>
              <Link href="/" className="inline-flex justify-center rounded-md border border-fg/15 text-platinum font-semibold hover:bg-fg/5 transition-colors px-6 py-3">
                Return Home
              </Link>
            </div>
          </motion.div>
