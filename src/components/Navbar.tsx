...
export function Navbar() {
  ...
  const menus = {
    methodology: {
      ...
      href: "/methodology",
      content: (
        <div className="grid grid-cols-3 gap-8 p-8 w-[780px]">
          {["1", "2", "3"].map((n) => (
            <div key={n} className="space-y-2">
              <span className="eyebrow text-cyan">0{n}/</span>
              <h4 className="text-platinum font-semibold text-sm">{t(`methodology.${n}.title`)}</h4>
              <p className="text-xs text-slate leading-relaxed">{t(`methodology.${n}.desc`)}</p>
            </div>
          ))}
        </div>
      ),
    ...
    suites: {
      ...
      href: "/suites",
      content: (
        <div className="w-[780px] p-8 grid grid-cols-3 gap-6">
          {ecosystem.map((suite) => (
            <Link
              key={suite.slug}
              href="/suites"
            ...
            >
              <h4 className="text-platinum font-semibold text-sm group-hover:text-cyan transition-colors flex items-center justify-between">
              ...
              <p className="text-xs text-slate leading-relaxed">{suite.products.length} modules</p>
            </Link>
          ))}
        </div>
      ),
    ...
    products: {
      ...
      href: "/products",
      content: (
        <div className="w-[820px] p-8 grid grid-cols-4 gap-6 max-h-[400px] overflow-y-auto">
          {ecosystem.map((suite) => (
            <div key={suite.slug} className="space-y-3">
              <h4 className="text-xs font-bold text-cyan tracking-wide uppercase">
                {language === "ar" && suite.suiteAr ? suite.suiteAr : suite.suite}
              </h4>
    ...
    about: {
      ...
      content: (
        <div className="p-8 w-[340px] space-y-4">
          <h4 className="text-platinum font-semibold text-sm">{t("nav.about.title")}</h4>
          <p className="text-xs text-slate leading-relaxed">{t("nav.about.desc")}</p>
          <Link
            href="/about"
            className="inline-flex items-center gap-1.5 text-xs text-cyan font-semibold hover:text-amethyst transition-colors"
            onClick={() => setActiveMenu(null)}
          >
  ...
  return (
    <header
      className={`sticky top-0 z-50 w-full transition-all duration-300 ${
        scrolled
          ? "bg-void/80 backdrop-blur-xl border-b border-fg/10"
          : "bg-void/0 border-b border-transparent"
      }`}
    >
      <div className="container mx-auto px-6 h-20 flex items-center justify-between relative">
            ...
            >
              ...
              <AnimatePresence>
                {activeMenu === key && (
                  ...
                  >
                    <div className="bg-void border border-fg/10 rounded-2xl shadow-xl shadow-black/5 overflow-hidden">
                      {menu.content}
                    </div>
                  </motion.div>
                )}
        ...
        <div className="flex items-center gap-3 z-10">
          ...
          <Link
            href="/audit"
            className="hidden md:inline-flex items-center gap-2 px-5 py-2.5 bg-cyan text-white rounded-md text-sm font-semibold hover:bg-amethyst transition-colors"
          >
            {t("nav.audit")}
          </Link>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="lg:hidden border-t border-fg/10 bg-void overflow-hidden"
          >
            <nav className="container mx-auto px-6 py-6 flex flex-col gap-1">
              {Object.entries(menus).map(([key, menu]) => (
                <Link
                  ...
                  className="py-3 text-base font-medium text-platinum hover:text-cyan transition-colors border-b border-fg/5"
                >
                  {menu.title}
                </Link>
              ))}
              <div className="pt-4 flex items-center gap-4">
                <Link
                  href="/audit"
                  onClick={() => setMobileOpen(false)}
                  className="flex-1 text-center py-3 bg-cyan text-white rounded-md font-semibold text-sm"
                >
                  {t("nav.audit")}
                </Link>
                <button
